...
use crate::miner::SHOULD_STOP_MINING;
...
impl Reader {
    pub fn stop_reading(&self) {
        debug!("READER STOP: Sending interrupts to {} reader threads", self.interupts.len());
        let mut sent_count = 0;
        for interrupt in &self.interupts {
            if interrupt.send(()).is_ok() {
                sent_count += 1;
            }
        }
        debug!("READER STOP: Sent {} interrupt signals", sent_count);
    }
    pub fn new(
        drive_id_to_plots: HashMap<String, Arc<Vec<Mutex<Plot>>>>,
        total_size: u64,
    ...
    fn create_read_task(
        ...
        trace!("🤔 READER DEBUG: Creating read task for drive={}, plot_count={}, height={}, scoop={}", 
        drive, plots.len(), height, scoop);
        ...
        (tx_interupt, move || {
            if SHOULD_STOP_MINING.load(Ordering::Relaxed) {
                info!("Reader task for drive {} skipped: ANNODE is not MINING", drive);
                return;
            }
            ...
            'outer: for (i_p, p) in plots.iter().enumerate() {
                if SHOULD_STOP_MINING.load(Ordering::Relaxed) {
                    info!("Reader interrupted for drive {}: ANNODE status changed to non-MINING", drive);
                    break 'outer;
                }
                let mut p = p.lock().unwrap();
                 trace!("🤔 READER DEBUG: Starting plot={} ({}/{}), account_id={}, total_nonces={}", 
        p.meta.name, i_p + 1, plot_count, p.meta.account_id, p.meta.nonces);
                ...
                'inner: for mut buffer in rx_empty_buffers.clone() {
                    if SHOULD_STOP_MINING.load(Ordering::Relaxed) {
                        info!("Reader stopping for drive {}: ANNODE is not MINING", drive);
                        buffer.unmap();
                    ...
                    let finished = i_p == (plot_count - 1) && next_plot;
                                      trace!("🤔 READER DEBUG: Read from plot={}, start_nonce_returned={}, account_id={}, scoop={}, bytes={}, finished={}", 
    p.meta.name, start_nonce, p.meta.account_id, scoop, bytes_read, finished);
                    if SHOULD_STOP_MINING.load(Ordering::Relaxed) {
                        info!("Reader stopping for drive {}: ANNODE status changed, not sending buffer", drive);
                        buffer.unmap();
                        tx_empty_buffers.send(buffer).unwrap();
                        break 'outer;
                    }
