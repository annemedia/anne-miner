...
pub fn gpu_info(cfg: &Cfg) {
    if cfg.gpu_worker_task_count > 0 {
        let platform_ids = core::get_platform_ids().unwrap_or_else(|e| {
            error!("GPU not available: {}", e);
            error!("Install GPU drivers or set 'gpu_worker_task_count = 0'");
            process::exit(1);
        });
...
impl GpuBuffer {
    pub fn new(context: &Arc<GpuContext>, id: usize) -> Self {
        if context.mapping {
            ...
            let buffer_size = (SCOOP_SIZE as usize) * context.gdim1[0];
            
            let layout = Layout::from_size_align(buffer_size, page_size::get()).unwrap();
            let pointer = unsafe { alloc(layout) };
            let data_vec = unsafe { Vec::from_raw_parts(pointer as *mut u8, buffer_size, buffer_size) };
...
fn transfer_buffer_to_gpu(gpu_context: &Arc<GpuContext>, buffer: &GpuBuffer, blocking: bool) {
    ...
    if gpu_context.mapping {
        ...
        if let Some(mapped) = &buffer.buffer_ptr_host {
            // Copy from host buffer to mapped GPU memory
            unsafe {
                let dst_ptr = mapped.as_ptr() as *mut u8;
                std::ptr::copy_nonoverlapping(data2.as_ptr(), dst_ptr, data2.len());
            }
            ...
            error!("GPU transfer in mapping mode but buffer not mapped!");
        }
    } else {
        unsafe {
            core::enqueue_write_buffer(
