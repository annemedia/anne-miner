...
impl Plot {
    pub fn new(path: &PathBuf, use_direct_io: bool, dummy: bool) -> Result<Plot, Box<dyn Error>> {
        if !path.is_file() {
    ...
    pub fn prepare(&mut self, scoop: u32) -> io::Result<u64> {
        ...
        debug!("[PLOT_PREPARE] file={}, scoop={}, nonces={}, initial_seek_addr={}, use_direct_io={}", 
           self.meta.name, scoop, nonces, seek_addr, self.use_direct_io);
    ...
    pub fn read(&mut self, bs: &mut Vec<u8>, scoop: u32) -> Result<(usize, u64, bool), io::Error> {
        ...
        trace!("[PLOT_READ] file={}, scoop={}, read_offset={}, calculated_seek={:?} (offset={}), bytes_to_read={}, start_nonce={}", 
           self.meta.name, scoop, self.read_offset, seek_addr, offset, bytes_to_read, start_nonce);
        if !self.dummy {
    ...
    pub fn seek_random(&mut self) -> io::Result<u64> {
        ...
        let rand_scoop = rng.random_range(0..SCOOPS_IN_NONCE);
