...
static mshabal128_context global_128_avx;
static mshabal128_context_fast global_128_fast_avx;

void init_shabal_avx() {
    mshabal_init_avx(&global_128_avx, 256);
    global_128_fast_avx.out_size = global_128_avx.out_size;
    for (uint64_t i = 0; i < 176; i++) global_128_fast_avx.state[i] = global_128_avx.state[i];
    global_128_fast_avx.Whigh = global_128_avx.Whigh;
    global_128_fast_avx.Wlow = global_128_avx.Wlow;
}
...
void find_best_deadline_avx(char *scoops, uint64_t nonce_count, char *gensig,
    ...
    memcpy(&x, &global_128_fast_avx, sizeof(global_128_fast_avx));
